...
- **Mismatch guard:** explicit subject conflicts are refused even when a candidate is semantically nearby.
- **Honest no-match state:** a score below the floor returns `no_match` with a reason instead of forcing a weak image.
- **Durable batches:** SQLite job rows track attempts, locks, retry delays, completion, and errors.
- **Cost ledger:** every vision and embedding call records provider, units, unit cost, total, and reference.
...
low-confidence count, tracked call count, seed-provider cost, and eval precision.
...
evaluation path, not claimed model photographs. Five labeled post cases cover
exact match, scientific-name paraphrase, another bird subject, another big cat,
and a no-match architecture post.
...
Current deterministic eval: **5/5, top-1 precision 1.00**.
...
| GET | `/api/costs` | cost ledger summary |
| GET | `/api/eval` | labeled eval result |
...
```text
images -> SQLite jobs -> VisionProvider -> validated tags -> image vectors
posts  ---------------------------------> post vectors
post + images -> cosine rank -> subject/confidence/score guard
...
config/         guard thresholds, aliases, pricing
fixtures/       50-image catalog, posts, labeled eval cases
...
services/       classification, embedding, matching, guard, eval, worker
scripts/        corpus generation, pipeline, eval, screenshot capture
...
- The checked-in corpus uses generated SVG stand-ins so the repo remains cheap,
  ...
  accuracy.
- SQLite job claiming is appropriate for this single-instance Capstone. A
  distributed deployment needs transactional row locking in Postgres.
